...
public final class NcDispatcherDecoder {

    private static final byte[] MAGIC = new byte[] { 0x72, 0x71, (byte) 0x89 };

    private NcDispatcherDecoder() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.err.println("Usage: NcDispatcherDecoder <raw-http-file> <client-home>");
            System.exit(2);
        }
    ...
    private static void decodeMessage(int messageNo, byte[] raw, int bodyStart, int bodyLength) {
        try {
            ...
            System.out.println("  DECODE_ERROR " + error.getClass().getName()
                    + ": " + error.getMessage());
        }
    }
        ...
        for (String name : names) {
            try {
                ...
                System.out.println("  " + name + "="
                        + (isSensitiveProperty(name) ? "<redacted>" : describe(property)));
    ...
    private static boolean isSensitiveProperty(String name) {
        return "getUserId".equals(name)
                || "getUserCode".equals(name)
                || "getCallId".equals(name);
    }
    ...
    private static void printResultFields(Object value) {
        ...
        try {
            ...
            System.out.println("  appexception=" + describe(exceptionValue));
        } catch (Throwable error) {
